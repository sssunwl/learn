...
- **泰文學科**(2026-09-24 派工):從零基礎+旅遊實戰,派俾 Codex 做,工單 `docs/CODEX_THAI.md`。會新開 `data-thai.js`,`app.js` 嘅 `speak()` 會加 `lang` 參數(唔再寫死 `ja-JP`),`SUBJECTS` 會多一個 `japanese` 同級嘅 `thai` subject。Codex 做完由 Claude review 先 commit/push,唔會自動上線
